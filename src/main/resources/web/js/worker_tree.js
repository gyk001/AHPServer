...
//将状态信息填充到树
function fillStateToTree (result) {
				$('#_worker_tree').children().remove();
				var workerList = [];
				...
				$("#_worker_tree").jstree(json_data);
				/*
				.bind("select_node.jstree", function (e, data) {
					alert(jQuery.data(data.rslt.obj[0], "id")); 
				});
				*/
				$('#tree_state_msg').text('已刷新!'+getNowTime());
}

//刷新状态树
function reloadTreeWorkerState(){
	"use strict";
	$('#tree_state_msg').text('刷新中...'+getNowTime());
	getWorkerState( fillStateToTree, function(msg){
		$('#tree_state_msg').text('刷新失败['+msg+']!'+getNowTime());
	});
}
$(function() {
	"use strict";
